...
const ForgotPassword = ({ countries }) => {
  ...
  const handleSubmit = (evt) => {
    ...
    setLoading(true);
    axios
      .post(`${apiData}user/forgot/password`, { phone: phoneWithCountryCode })
      .then((res) => {
        if (res.data.status) {
          setLoading(false);
          setCodeSent(true);
      ...
      .catch((error) => {
        console.log(
          `Error getting password forgot code due to: ${error.message}`
        );
        setLoading(false);
      });
  };
  ...
  const handleSubmitForNewPassword = (evt) => {
    ...
    setLoading(true);
    axios
      .post(`${apiData}user/password/reset`, formFields)
      .then((res) => {
        if (res.data.status) {
          router.push("/login");
        } else {
                  ...
                  <>
                    <div className="flex flex-col md:flex-row">
                      <div className="mr-2 pt-2 w-auto">
                        <label id="phone" className="w-full">
                          Phone Number:{" "}
                        </label>
                      </div>
                      <div
                        className="absolute mt-10 md:mt-2 left-[6%] md:left-[31%] flex space-x-1 items-center justify-center ml-8 hover:cursor-pointer"
                      ...
                      <div className="w-[100%] md:w-[60%] mb-3 md:mb-0 md:mr-2">
                        <input
                          type="text"
                          value={phone}
                          onChange={handleChange}
                          htmlFor="phone"
                          className="w-full rounded-lg py-2 pl-[112px] sm:pl-[98px] pr-3 outline outline-[#F1F1F1] focus:outline-[#FFA500]"
                        />
                      </div>
                  ...
                  <>
                    <div className="flex flex-col md:flex-row my-3 md:my-9">
                      <div className="flex flex-col flex-auto md:mr-6 mb-4 md:mb-0">
                        <div>
                          <label id="token" className="w-full">
                            code:{" "}
                          </label>
                        </div>
                        <div className="w-full mb-3 md:mb-0 md:mr-2">
                          <input
                            ...
                            onChange={handleChangeForNewPassword("token")}
                            htmlFor="token"
                            className="w-full rounded-lg py-2 px-4 outline outline-[#F1F1F1] focus:outline-[#FFA500]"
                          />
                        </div>
                      </div>
                      <div className="flex flex-col flex-auto mb-4 md:mb-0">
                        ...
                        {showPassword ? (
                          <BsEyeSlash
                            ...
                            className={
                              showIcon
                                ? "absolute  mt-9  right-[10%] md:right-[25%] hover:cursor-pointer"
                                : "hidden"
                            }
                          />
                        ) : (
                        ...
                        <div className="w-full mb-3 md:mb-0 md:mr-2">
                          <input
                            ...
                            onChange={handleChangeForNewPassword("password")}
                            htmlFor="newpassword"
                            className="w-full rounded-lg py-2 px-4 outline outline-[#F1F1F1] focus:outline-[#FFA500]"
                          />
                        </div>
                    ...
                    <div className="w-[100%] md:w-[50%] mx-auto">
                      <button
                        ...
                        className={
                          !isLoading
                            ? "w-full bg-[#FFA500] hover:bg-[#fabe50] text-white px-4 py-2 text-center rounded-lg"
