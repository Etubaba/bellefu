...
export default function UnstyledSelectSimple3({ countryStuffs, catchLgas }) {
  const [newstatearr, setNewStateArr] = useState(null);
  ...
  useEffect(() => {
    axios
      .get(
        `https://bellefu.inmotionhub.xyz/api/web30/get/postadd/states/${userThing?.country_code}`
      )
      .then((response) => {
        setNewStateArr(response?.data.state);
      })
      .catch((error) => {
        console.log(error);
      });
